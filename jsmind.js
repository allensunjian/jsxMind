; (function ($w) {
  ...
  jm.node.loading = (function () {
    var d = $c('div'),
      img = $c('img');
    d.className = 'hm-loading-show';
    img.className = 'jm-loading';
  ...
  // ============= 右键  =============================================
  jm.menu = function (scope, opts) {
    this.scope = scope;
    this.opts = opts;
    this.menuItemList = opts.injectionList || ['edit', 'addChild', 'addBrother', 'delete'];
    var _this = this;
    this.funcMap = {
      edit: {
        fn: this.scope.begin_edit.bind(this.scope),
        text: '编辑节点'
      },
      addChild: {
        fn: function () {
          _this.scope.shortcut.handle_addchild.call(_this.scope.shortcut, _this.scope)
        },
        text: '添加子节点'
      },
      addBrother: {
        fn: function () {
          _this.scope.shortcut.handle_addbrother.call(_this.scope.shortcut, _this.scope)
        },
        text: '添加兄弟节点'
      },
      delete: {
        fn: function () {
          _this.scope.shortcut.handle_delnode.call(_this.scope.shortcut, _this.scope)
        },
        text: '删除节点'
      }
    };
  ...
  jm.menu.prototype = {
    ...
    selected_node: null,
    init_menu: function () {
      this.menu_events_init();
    },
    menu_events_init() {
      var _this = this;
      this.create_menu_panel()
      window.oncontextmenu = function (e) {
        e.preventDefault();
        _this.menu_node.style.left = e.clientX + 'px';
        _this.menu_node.style.top = e.clientY + 'px';
        _this.menu_node.style.display = 'block';
        _this.selected_node = _this.get_selected_node();
      }
      window.onclick = function (e) {
        _this.menu_node.style.display = 'none';
      }
      this.menuItemList.forEach(function (k) {
        var o = _this.funcMap[k];
        _this.create_menu_item(o.text, o.fn);
      })
    },
    get_selected_node() {
        return this.scope.get_selected_node();
    },
    create_menu_panel() {
      var p = $c('div')
      p.className = 'jm-menu';
      p.id = 'menu';
      this.menu_node = p;
      this.scope.view.e_panel.appendChild(p)
    },
    create_menu_item(text, fn, node) {
      var item = $c('div'), _this = this;
      item.className = 'jm-menu-item';
      $h(item, text)
      item.addEventListener('click', function () {
        ...
        fn(_this.selected_node)
      })
      this.menu_node.appendChild(item)
    },
    assemble_menu() {
  ...
  jm.prototype = {
    init: function () {
      ...
      this.menu = opts.profileOpts && opts.profileOpts.showMenu && new jm.menu(this, opts.menu || {});

      /**
* modified by Allen.sun on 2019/11/22
* Note: 增加一个trigger接口 用以扩展
**/
      jm.init_add_modal.call(this);
      jm.init_triggerEvent.call(this);
    ...
    get_profile_options() {
      return this.options.profileOpts ? this.options.profileOpts : {}
    },
  ...
  jm.view_provider.prototype = {
    toggle_add_modal(type) {
      var isShow = type == 'show';
      this.e_add_modal.style.display = isShow ? 'block' : 'none';
    },
  ...
  jm.shortcut_provider.prototype = {
    ...
    handle_addchild: function (_jm, e, next) {
      var selected_node = _jm.get_selected_node(),
        selected_id = selected_node.id;
      if (!!selected_node) {
        var useDefaultId = _jm.get_profile_options().useDefaultId;
        if (!useDefaultId && typeof useDefaultId !== 'undefined') {
          next(selected_node, _jm.add_node)
          return;
        }
        var nodeid = jm.util.uuid.newid();
        var node = _jm.add_node(selected_node, nodeid, '点击编辑');
        if (!!node) {
    ...
    handle_addbrother: function (_jm, e, next) {
      var selected_node = _jm.get_selected_node(),
        selected_id = selected_node.id;
      if (!!selected_node && !selected_node.isroot) {
       var useDefaultId = _jm.get_profile_options().useDefaultId;
        if (!useDefaultId && typeof useDefaultId !== 'undefined') {
         next(selected_node, _jm.insert_node_after)
         return;
       }
        var nodeid = jm.util.uuid.newid();
        var node = _jm.insert_node_after(selected_node, nodeid, '点击编辑');
        if (!!node) {
          _jm.select_node(nodeid);
          _jm.begin_edit(nodeid);
        }
    ...
    handle_delnode: function (_jm, e) {
      var selected_node = _jm.get_selected_node(),
        ...
        nodePic = selected_node.topic;
      if (!!selected_node && !selected_node.isroot) {
        _jm.select_node(selected_node.parent);
        _jm.remove_node(selected_node);
      };
      return { id: nodeId, topic: nodePic};
    },
  ...
  jm.register_triggerEvent = function (name, type, fn) {
    jm.trigger.push({
      name: name,
      type: type,
      fn: fn
    ...
    //  init trigger queue
  jm.init_triggerEvent = function () {
    var l = jm.trigger,
      _this = this;
    l.forEach(function (o) {
      // node__pos trigger
      if (o.type == "node__pos") {
        var fn = _this.view.select_node;
        _this.view.select_node = function (node) {
          fn.call(_this.view, node)
          if (node) o.fn(node)
        }
      }
      // node__add trigger
      if (o.type == "node__add") {
        var callList = ['handle_addchild','handle_addbrother'],
          editEnd = _this.view.edit_node_end,
          newNodeObj = null,
          selectedNode = null,
          createNodeMethods = null;
        _this.view.edit_node_end = function () {
          var node = editEnd.apply(_this.view, arguments);
          if (!node) return;
          var elem = node._data.view.element,
            loadingNode = jm.node.loading;
          // newNodeObj 新增的页面节点， 如果没有新增节点信息则是编辑，有新增节点则是 新增节点的编辑
          elem.appendChild(loadingNode);
          o.fn(_this.shortcut, function (opts) {
            var formatAsync = opts.formatAsnc || false,
              requestDataEdit = opts.requestDataEdit,
              url = opts.editUrl,
              send = opts.send,
              retFunc = opts.retFunc || function () { },
              ajaxReName = opts.ajaxReName || function () { };
            function sendEdit(requestDataEdit,fn) {
              jm.util.ajax.post(url, requestDataEdit, function (res) {
                fn(res)
              }, function () {
            ...
            if (!formatAsync) {
              ...
              retFunc(newNodeObj ? newNodeObj : node);
              newNodeObj = null;
            } else {
              ajaxReName(sendEdit, node, editnext, editprev())
            }
          })
        }
     // --------------------------- add start--------------------------------------------------
        _this.view.e_modal_btn.addEventListener('click', modal_handle);
        _this.view.e_modal_cancel.addEventListener('click', modal_cancel);
        function next(val) {
          return function (id) {
            var node = createNodeMethods.apply(_this, [selectedNode, id, val]);
            if (!!node) {
              _this.view.select_node.call(_this, node);
            }
            _this.view.toggle_add_modal('hide');
          }
        };
        function prev() {
          _this.view.toggle_add_modal('hide');
        };
        function checkVal(val,fn,el) {
        ...
        function addNode(opts) {
          return function (requestDataAddNew, fn) {
            jm.util.ajax.post(opts.addUrl, requestDataAddNew, function (res) {
              fn(res)
            })
          }
        }
        function modal_handle(e) {
          var input = e.target.previousElementSibling,
            val = input.value;
          o.fn(_this.shortcut, function (opts) {
            checkVal.call(opts, val, function (data) {
              opts.reIdFunc(addNode(opts), data, _this.get_selected_node(), next(val), prev)
            },  input)
          })
          e.preventDefault();
        };
        ...
        callList.forEach(function (k) {
          var calledFn = _this.shortcut[k];
          jm.shortcut_provider.prototype[k] = function (args) {
            var agrsArr = [args, null, function (selected_node, fn) {
              selectedNode = selected_node;
              createNodeMethods = fn;
              _this.view.toggle_add_modal('show');
            }];
            newNodeObj = calledFn.apply(_this.shortcut, agrsArr);
          }
        })
      }
      // node__del
      if (o.type == 'node__del') {
        var fn = _this.shortcut.handle_delnode;
        _this.shortcut.handle_delnode = function () {
          var node = fn.call(_this.shortcut, _this)
          if (node.id) o.fn(node)
        }
      } 
      // node__drag 
      if (o.type == 'node__drag') {
        var fn = _this.move_node;
        _this.move_node = function () {
  ...
  jm.init_add_modal = function () {
    ...
    var b_c = $c('div');
    var mb = $c('div');
    b_c.className = 'jm__add-modal-btn jm__add-modal-cancel';
    mb.className = 'jm__add-modal-bg';
    s.innerText = "添加节点";
    b_c.innerText = '取消';
    s.className = "jm__add-modal-text";
    input.className = "jm__add-modal-input";
    p.className = 'jm__add-modal';
    b.className = 'jm__add-modal-btn';
    b.innerText = '确定';
    p.appendChild(s);
    p.appendChild(input);
    p.appendChild(b);
    p.appendChild(b_c);
    mb.appendChild(p);
    this.view.e_panel.appendChild(mb)
    this.view.e_add_modal = mb;
    this.view.e_modal_btn = b;
    this.view.e_modal_cancel = b_c;
  };
